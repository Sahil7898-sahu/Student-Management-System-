// Perfect Final Student Management System - Google Sheets Integration - All Errors Fixed
class PerfectFinalSystem {
    constructor() {
        this.students = [];
        this.deskCharge = 500;
    ...
    updateStats() {
        try {
            ...
            const totalRevenue = this.students.reduce((sum, student) => {
                ...
                return sum + studentPayments.reduce((paymentSum, payment) => paymentSum + (payment.amount || 0), 0);
            }, 0);
    ...
    displayRecentStudents() {
        try {
            ...
            container.innerHTML = recent.map(student => `
                <div class="recent-student" onclick="perfectFinalSystem.showStudentDetails('${student.id}')">
            ...
            const tableHTML = `
                <div class="table-header">
                    ...
                    <div class="table-actions">
                        <button class="btn-primary" onclick="perfectFinalSystem.displayStudentsTable()">
                            ...
                            ${sortedStudents.map(student => {
                                ...
                                if (student.payments && Array.isArray(student.payments) && student.payments.length > 0) {
                                    try {
                                        ...
                                        if (sortedPayments.length > 0 && sortedPayments[0] && sortedPayments[0].amount) {
                                            latestPaymentInfo = `₹${sortedPayments[0].amount.toLocaleString()}`;
                                ...
                                return `
                                    <tr>
                                        ...
                                        <td>
                                            <button class="student-name-btn" onclick="perfectFinalSystem.showStudentDetails('${student.id}')">
                                        ...
                                        <td>
                                            <button class="btn-primary btn-sm" onclick="perfectFinalSystem.showStudentDetails('${student.id}')">
                                                <i class="fas fa-eye"></i> View
                                            </button>
                                            <button class="btn-danger btn-sm" onclick="perfectFinalSystem.showRemoveStudentModal('${student.id}')">
                        ...
                        <div class="student-detail-grid">
                            ...
                            <div class="detail-section">
                                <div class="action-buttons">
                                    <button class="btn-primary" onclick="perfectFinalSystem.showAdminPaymentModal('${student.id}')">
                                        <i class="fas fa-plus"></i> Add Payment
                                    </button>
                                    <button class="btn-danger" onclick="perfectFinalSystem.showRemoveStudentModal('${student.id}')">
            ...
            modal.innerHTML = `
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-actions">
                            <button type="button" class="btn-danger" onclick="perfectFinalSystem.confirmRemoveStudent('${studentId}')">
                                <i class="fas fa-user-times"></i> Remove Student
                            </button>
                            <button type="button" class="btn-secondary" onclick="this.closest('.modal').remove()">
    ...
    confirmRemoveStudent(studentId) {
        try {
            ...
            const removalRecord = {
                ...
                totalAmount: student.payments ? student.payments.reduce((sum, p) => sum + (p.amount || 0), 0) : 0
            };
            ...
            // Try to sync with Google Sheets
            if (this.isOnline) {
                this.makeRequest('removeStudent', { studentId })
                    .then(response => {
                        if (response.status === 'success') {
                    ...
                    .catch(error => {
                        this.showMessage(`⚠️ ${student.name} removed locally. Desk ${student.deskNumber} is now available.`, 'warning');
                    });
            } else {
                this.showMessage(`✅ ${student.name} removed successfully! Desk ${student.deskNumber} is now available.`, 'success');
            }
    ...
    exportToJSON() {
        try {
            ...
            a.href = url;
            a.download = `perfect-final-student-data-${new Date().toISOString().split('T')[0]}.json`;
            a.click();
...
document.addEventListener('DOMContentLoaded', function() {
    try {
        window.perfectFinalSystem = new PerfectFinalSystem();
    } catch (error) {
